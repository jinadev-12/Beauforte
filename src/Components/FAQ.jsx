...
export default function FAQ() {
  ...
  const toggle = async (idx) => {
    ...
    if (isOpen) {
      // close
      gsap.to(ans, { height: 0, opacity: 0, duration: 0.35, ease: "power2.out" });
      setOpenIndex(null);
    } else {
      ...
      gsap.to(ans, { height: targetHeight, opacity: 1, duration: 0.35, ease: "power2.out" });
      setOpenIndex(idx);
  ...
  return (
    <div id="FAQ" ref={faqRef} className="py-20 px-8 md:px-20 lg:px-32 font-lato bg-white">
            ...
            {faqs.map((faq, index) => (
              <div key={index} className="faq-item border-b border-gray-100 py-4">
                <div className="flex items-center justify-between py-2">
                  ...
                  >
                    <h3 className="text-base md:text-lg font-light text-gray-900">{faq.question}</h3>
                    <span className="ml-4 text-gray-500 text-lg">{openIndex === index ? "−" : "+"}</span>
                  </button>
                </div>
